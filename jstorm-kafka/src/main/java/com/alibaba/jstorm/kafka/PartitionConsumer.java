...
public class PartitionConsumer {
	...
	public PartitionConsumer(MessageConsumer consumer, KafkaSpoutConfig config, Partition partition) {
		this.config = config;
		...
		if(config.fromBeginning) {
		    ...
		    //consumer.getConsumer().getOffsetsBefore(request) getOffsetsBefore(config.topic, partition.getPartition(), config.startOffsetTime, 1)[0];
		}
	}
